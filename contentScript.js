...
var parseQuestion = async function() {
    ...
    var prompt = document.querySelector('.prompt').innerText.split('\n').map(x => x.trim()).filter(Boolean).join(" ")
    var clutter = prompt.substring(prompt.lastIndexOf(" \n") + 1, prompt.lastIndexOf("\n ") + 1)
    prompt.split(clutter).map(x => x.trim()).join(" ")
    return { type, prompt }
}
...
var saveAnswer = async function(question) {
    ...
    if(question.type.includes("Fill in the Blank")) 
        ...
        answers = document.querySelector('.answer-container').innerText.split('\n').filter(Boolean)
    else{
...
if(window.location.href.includes('https://lms.mheducation.com/mghmiddleware')){
    ...
    sleep(3000).then(() => {
        if(document.querySelector('#ezt_assignment_category_e').innerText == "SmartBook 2.0"){
...
//If reaches the start page!
    ...
    sleep(3000).then(() => {
        if(document.querySelector('.welcome-learn__heading') != null) {
            document.querySelector('.welcome-learn__heading').innerText = "Welcome home Cheater!";
        }
        let elements = document.getElementsByTagName("button");
        for(let i = 0, len = elements.length; i < len; i++) {
            if(elements[i].innerText == "Continue Questions" || elements[i].innerText == "Start Questions"){
